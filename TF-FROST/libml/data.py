...
_DATA_CACHE = None
SAMPLES_PER_CLASS = [1, 2, 3, 4, 5, 8, 10, 12, 16, 20, 25, 32, 40, 64, 100, 128, 144, 256, 400, 512]
...
flags.DEFINE_string('data_subfolder', 'SSL2','Subfolder for the data')
...
class DataSets:
    def __init__(self, name, train_labeled: DataSet, train_unlabeled: DataSet, test: DataSet, valid: DataSet, train_original: DataSet,
        ...
        self.subfolder = FLAGS.data_subfolder
...
#        root = os.path.join(DATA_DIR, 'SSL2', name)
        root = os.path.join(DATA_DIR, FLAGS.data_subfolder, name)
...
def create_datasets(augment_fn):
    ...
    d.update([DataSets.creator('cifar10imb', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(10), [10 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
    ...
    d.update([DataSets.creator('cifar100', seed, label, valid, augment_fn, nclass=100)
              for seed, label, valid in itertools.product(range(6), [100 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
