...
#                    color=policies.color_policies(),
                    cifar10imb=policies.cifar10_policies(), cifar100=policies.cifar10_policies(), 
...
AUGMENT_ENUM = 'd x m aa aac aacc ra rac'.split() + ['r%d_%d_%d' % (nops, mag, cutout) for nops, mag, cutout in
...
class AugmentPoolAA(AugmentPool):
    ...
    def __init__(self, get_samples, policy_group):
        ...
        self.queue = []
        self.fill_queue()
        print("AugmentPoolAA:  policy_group ", policy_group) 
    ...
    def queue_images(self, batch):
        args = []
        image = batch['image']
        print("AugmentPoolAA:  policy_group ", policy_group, POLICIES[self.policy_group]) 
        if image.ndim == 4:
...
DEFAULT_AUGMENT = EasyDict(
    cifar10=AugmentPair(tf=lambda x: dict(image=Primitives.ms(4)(x), label=x['label'], index=x.get('index', -1)),
                        numpy=AugmentPool),
    cifar10imb=AugmentPair(tf=lambda x: dict(image=Primitives.ms(4)(x), label=x['label'], index=x.get('index', -1)),
                        numpy=AugmentPool),
...
AUTO_AUGMENT_COLOR = EasyDict({
    k: AugmentPair(tf=v.tf, numpy=functools.partial(AugmentPoolAAC, policy_group='color'))
    for k, v in DEFAULT_AUGMENT.items()
})
...
def get_augmentation(dataset: str, augmentation: str):
    ...
    elif augmentation == 'aacc':
        return AUTO_AUGMENT_COLOR[dataset]
