...
class WideResnet(nn.Module):
    ...
    '''
    def __init__(self, n_classes, k=1, n=28, feature_dim=128):
        super(WideResnet, self).__init__()
        self.n_layers, self.k = n, k
        self.backbone = WideResnetBackbone(k=k, n=n)
        self.classifier = nn.Linear(64 * self.k, n_classes, bias=True) #can be deleted
        # projection head
        self.g = nn.Sequential(nn.Linear(8192, 512, bias=False), nn.BatchNorm1d(512),
                               nn.ReLU(inplace=True), nn.Linear(512, feature_dim, bias=True))

    def forward(self, x):
        feat = self.backbone(x)[-1]
        fv = torch.flatten(feat, start_dim=1)
        feat = torch.mean(feat, dim=(2, 3))
        feat = self.classifier(feat)
        z = self.g(fv)
        #print(feat.shape)
        
        return feat, F.normalize(fv, dim=-1), F.normalize(z, dim=-1)
